...
cvar_firegrenades, cvar_frostgrenades, cvar_survgravity,
...
cvar_flashshowall, cvar_allowrespawninfection,
cvar_survinfammo, cvar_nemknockback,
...
public event_ammo_x(id)
{
	...
	// Unlimited BP Ammo?
	if (g_class[id] >= SURVIVOR && g_class[id] < ZOMBIE ? get_pcvar_num(cvar_survinfammo) : get_pcvar_num(cvar_infammo))
	{
...
public fw_PlayerKilled(victim, attacker, shouldgib)
{
	...
	if (get_pcvar_num(cvar_deathmatch))
	{
		...
		if ((g_class[victim] == ZOMBIE && g_class[victim] < NEMESIS && !get_pcvar_num(cvar_respawnzomb)) || (g_class[victim] == HUMAN && g_class[victim] < SURVIVOR && !get_pcvar_num(cvar_respawnhum)) || (g_class[victim] >= NEMESIS && !get_pcvar_num(cvar_respawnnem)) || (g_class[victim] >= SURVIVOR && !get_pcvar_num(cvar_respawnsurv)))
			return;
...
show_menu_game(id)
{
	...
	if (get_pcvar_num(cvar_buycustom))
		...
		len += formatex(menu[len], charsmax(menu) - len, "\d1. \dComprar Armas^n");
	...
	//7
	len += formatex(menu[len], charsmax(menu) - len, "\r7. \wTop Players^n")
...
show_menu_extras(id)
{
	...
	// Title
	formatex(menu, charsmax(menu), "%L [%L]\r", id, "MENU_EXTRA_TITLE", id, g_class[id] == ZOMBIE ? g_class[id] == NEMESIS ? "CLASS_NEMESIS" : "CLASS_ZOMBIE" : g_class[id] == SURVIVOR ? "CLASS_SURVIVOR" : "CLASS_HUMAN")
...
public show_menu_zclass(id)
{
	...
	for (class = 0; class < g_zclass_i; class++)
	{
		...
		if( g_iLevel[id] >= ArrayGetCell(g_zclass_level, class)){
			...
			formatex(menu, charsmax(menu), "%s \r[ Level %d ]", buffer, ArrayGetCell(g_zclass_level, class))
		}
...
show_menu_admin(id)
{
	...
	if (userflags & (g_access_flag[ACCESS_MODE_INFECTION] | g_access_flag[ACCESS_MAKE_ZOMBIE] | g_access_flag[ACCESS_MAKE_HUMAN]) && g_ammopacks[id] >= 150)
		len += formatex(menu[len], charsmax(menu) - len, "\r1.\w %L \r[ 150 APS ]^n", id, "MENU_ADMIN1")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d1. %L \r[ 150 APS ]^n", id, "MENU_ADMIN1")
	
	// 2. Nemesis command
	if (userflags & (g_access_flag[ACCESS_MODE_NEMESIS] | g_access_flag[ACCESS_MAKE_NEMESIS]) && g_ammopacks[id] >= 300)
		len += formatex(menu[len], charsmax(menu) - len, "\r2.\w %L \r[ 300 APS ]^n", id, "MENU_ADMIN2")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d2. %L \r[ 300 APS ]^n", id, "MENU_ADMIN2")
	
	// 3. Survivor command
	if (userflags & (g_access_flag[ACCESS_MODE_SURVIVOR] | g_access_flag[ACCESS_MAKE_SURVIVOR]) && g_ammopacks[id] >= 300)
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\w %L \r[ 300 APS ]^n", id, "MENU_ADMIN3")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d3. %L \r[ 300 APS ]^n", id, "MENU_ADMIN3")
	
	// 4. Respawn command
	if (userflags & g_access_flag[ACCESS_RESPAWN_PLAYERS] && g_ammopacks[id] >= 75)
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w %L \r[ 75 APS ]^n", id, "MENU_ADMIN4")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d4. %L \r[ 75 APS ]^n", id, "MENU_ADMIN4")
	
	// 5. Swarm mode command
	if ((userflags & g_access_flag[ACCESS_MODE_SWARM]) && allowed_swarm() && g_ammopacks[id] >= 100)
		len += formatex(menu[len], charsmax(menu) - len, "\r5.\w %L \r[ 100 APS ]^n", id, "MENU_ADMIN5")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d5. %L \r[ 100 APS ]^n", id, "MENU_ADMIN5")
	
	// 6. Multi infection command
	if ((userflags & g_access_flag[ACCESS_MODE_MULTI]) && allowed_multi() && g_ammopacks[id] >= 150)
		len += formatex(menu[len], charsmax(menu) - len, "\r6.\w %L \r[ 150 APS ]^n", id, "MENU_ADMIN6")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d6. %L \r[ 150 APS ]^n", id, "MENU_ADMIN6")
	
	// 7. Plague mode command
	if ((userflags & g_access_flag[ACCESS_MODE_PLAGUE]) && allowed_plague() && g_ammopacks[id] >= 800)
		len += formatex(menu[len], charsmax(menu) - len, "\r7.\w %L \r[ 800 APS ]^n", id, "MENU_ADMIN7")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d7. %L \r[ 800 APS ]^n", id, "MENU_ADMIN7")
...
show_menu_player_list(id)
{
	...
	for (player = 0; player <= g_maxplayers; player++)
	{
		...
		switch (PL_ACTION)
		{
			...
			{
				if (g_class[player] >= ZOMBIE && g_ammopacks[id] >= 150)
				{
			...
			{
				if (allowed_nemesis(player) && (g_newround ? (userflags & g_access_flag[ACCESS_MODE_NEMESIS]) : (userflags & g_access_flag[ACCESS_MAKE_NEMESIS])) && g_ammopacks[id] >= 300)
				{
			...
			{
				if (allowed_survivor(player) && (g_newround ? (userflags & g_access_flag[ACCESS_MODE_SURVIVOR]) : (userflags & g_access_flag[ACCESS_MAKE_SURVIVOR])) && g_ammopacks[id] >= 300)
				{
					if (g_class[player] >= ZOMBIE)
						formatex(menu, charsmax(menu), "%s \r[%s]", g_playername[player], g_class[player] == NEMESIS ? "Nemesis" : "Zombie")
					else
			...
			{
				if (allowed_respawn(player) && (userflags & g_access_flag[ACCESS_RESPAWN_PLAYERS])  && g_ammopacks[id] >= 75)
...
public menu_admin(id, key)
{
	...
	switch (key)
	{
		...
		{
			if (userflags & g_access_flag[ACCESS_MODE_SWARM] && g_ammopacks[id] >= 100)
			{
				if (allowed_swarm())
		...
		{
			if (userflags & g_access_flag[ACCESS_MODE_MULTI] && g_ammopacks[id] >= 150)
			{
				if (allowed_multi())
		...
		{
			if (userflags & g_access_flag[ACCESS_MODE_PLAGUE] && g_ammopacks[id] >= 800)
			{
				if (allowed_plague())
	...
	if (g_isconnected[playerid])
	{
		...
		switch (PL_ACTION)
		{
			case ACTION_ZOMBIEFY_HUMANIZE: // Zombiefy/Humanize command
			{
				...
				{
					if (userflags & g_access_flag[ACCESS_MAKE_HUMAN] && g_ammopacks[id] >= 150)
					{
				...
				{
					if ((g_newround ? (userflags & g_access_flag[ACCESS_MODE_INFECTION]) : (userflags & g_access_flag[ACCESS_MAKE_ZOMBIE])) && g_ammopacks[id] >= 150)
					{
			...
			case ACTION_MAKE_NEMESIS: // Nemesis command
			{
				if ((g_newround ? (userflags & g_access_flag[ACCESS_MODE_NEMESIS]) : (userflags & g_access_flag[ACCESS_MAKE_NEMESIS])) && g_ammopacks[id] >= 300)
				{
			...
			case ACTION_MAKE_SURVIVOR: // Survivor command
			{
				if ((g_newround ? (userflags & g_access_flag[ACCESS_MODE_SURVIVOR]) : (userflags & g_access_flag[ACCESS_MAKE_SURVIVOR])) && g_ammopacks[id] >= 300)
				{
...
public message_cur_weapon(msg_id, msg_dest, msg_entity)
{
	...
	if (g_class[msg_entity] >= SURVIVOR && g_class[msg_entity] < ZOMBIE ? get_pcvar_num(cvar_survinfammo) <= 1 : get_pcvar_num(cvar_infammo) <= 1)
		return;
...
public message_teaminfo(msg_id, msg_dest)
{
	...
	switch (team[0])
	{
		...
		case 'T': // Terrorist
		{
			if ((g_currentmode == MODE_SWARM || g_currentmode == MODE_SURVIVOR) && fnGetHumans()) // survivor alive or swarm round w/ humans --> spawn as zombie
			{
				g_respawn_as_zombie[id] = true;
			}
...
zombieme(id, infector, nemesis, silentmode, rewards)
{
	...
	// Nemesis aura task
	if (g_class[id] >= NEMESIS && get_pcvar_num(cvar_nemaura))
		set_task(0.1, "zombie_aura", id+TASK_AURA, _, _, "b")
...
save_customization()
{
	...
	for (i = EXTRAS_CUSTOM_STARTID; i < size; i++)
	{
		if (ArrayGetCell(g_extraitem_new, i))
		{
			...
			// Add Level
			formatex(buffer, charsmax(buffer), "^nLevel = %d", ArrayGetCell(g_extraitem_level, i))
			fputs(file, buffer)
...
public respawn_player_task(taskid)
{
	...
	if (!g_endround && team != FM_CS_TEAM_SPECTATOR && team != FM_CS_TEAM_UNASSIGNED && !g_isalive[ID_SPAWN] && g_currentmode < MODE_NEMESIS /*(!g_survround || get_pcvar_num(cvar_allowrespawnsurv)) && (!g_swarmround || get_pcvar_num(cvar_allowrespawnswarm)) && (!g_nemround || get_pcvar_num(cvar_allowrespawnnem)) && (!g_plagueround || get_pcvar_num(cvar_allowrespawnplague))*/)
	{
		// Infection rounds = none of the above
		if (!get_pcvar_num(cvar_allowrespawninfection) && g_currentmode < MODE_NEMESIS)
			return;
...
frost_explode(ent)
{
	...
	while ((victim = engfunc(EngFunc_FindEntityInSphere, victim, originF, NADE_EXPLOSION_RADIUS)) != 0)
	{
		...
		if (g_handle_models_on_separate_ent)
			set_user_rendering(g_ent_playermodel[victim], kRenderFxGlowShell, 0, 100, 200, kRenderNormal, 25)
		else
...
reset_vars(id, resetall)
{
	...
	if (resetall)
	{
		...
		g_damagedealt[id] = 0
		WPN_AUTO_ON = 0
		g_iDamage[id] = g_iExp[id] = g_iReset[id] = 0; 
		g_iRango[id] = 0; 
		g_iLevel[id] = 1;
	}
}
...
public ShowHUD(taskid)
{
	...
	else // humans
	{
		if (g_class[id] == SURVIVOR)
			...
			copy(class, charsmax(class), rango[g_iRango[id]][range_name]);
	}
	...
	else
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "Vida: %d^nClase: %s^nExperiencia: %d^nNivel: %d/%d || Reset: %d^nAmmo packs: %d^nDamage: %d/%d", 
			get_user_health(id), class, g_iExp[id], g_iLevel[id], MAX_LEVEL, g_iReset[id], g_ammopacks[id], g_iDamage[id], DEFAULT_DAMAGE);
	}
}
	...
	if(g_ammopacks[id] >= cost)
	{
		switch(command)
		{
			case 0:{
				...
				zp_colored_print(0, "^x04[ZP] ^x01El Admin ^x04%s ^x01convirtio a ^x04%s ^x01en ^x04Zombie", g_playername[id], g_playername[player] );
			}
			case 1:{
				humanme(player, 0, 0)
			}
			case 2:{
				...
				zp_colored_print(0, "^x04[ZP] ^x01El Admin ^x04%s ^x01convirtio a ^x04%s ^x01en ^x04Humano", g_playername[id], g_playername[player] );
			}
			case 3:{
				// New round?
				if (g_newround)
				...
				zp_colored_print(0, "^x04[ZP] ^x01El Admin ^x04%s ^x01convirtio a ^x04%s ^x01en ^x04Nemesis", g_playername[id], g_playername[player] );
			}
			case 4:{
				// Respawn as zombie?
				if (get_pcvar_num(cvar_deathmatch) == 2 || (get_pcvar_num(cvar_deathmatch) == 3 && random_num(0, 1)) || (get_pcvar_num(cvar_deathmatch) == 4 && fnGetZombies() < fnGetAlive()/2))
				...
				zp_colored_print(0, "^x04[ZP] ^x01El Admin ^x04%s ^x01ha Revivido a ^x04%s", g_playername[id], g_playername[player] );
			}
...
public native_register_extra_item(const name[], cost, level, team)
{
	...
	for (i = 0; i < size; i++)
	{
		...
		// Replace level
		buffer[0] = ArrayGetCell(g_extraitem2_realname, i)
		ArraySetCell(g_extraitem_level, g_extraitem_i, buffer[0])
...
public native_register_zombie_class(const name[], const info[], const model[], const clawmodel[], level, hp, speed, Float:gravity, Float:knockback)
{
	// ZP disabled
	if (!g_pluginenabled)
		return -1;
...
public set_user_nvision(taskid)
{
	...
	if (g_class[ID_NVISION] >= NEMESIS)
	{
		write_byte(cvar_nemnvgcolor[0]) // r
		write_byte(cvar_nemnvgcolor[1]) // g
		write_byte(cvar_nemnvgcolor[2]) // b
	}
	// Human / Spectator in normal round
	else if (g_class[ID_NVISION] < ZOMBIE || !g_isalive[ID_NVISION])
	{
		write_byte(cvar_humnvgcolor[0]) // r
		write_byte(cvar_humnvgcolor[1]) // g
		write_byte(cvar_humnvgcolor[2]) // b
	}
	// Zombie
	else
	{
		write_byte(cvar_nvgcolor[0]) // r
		write_byte(cvar_nvgcolor[1]) // g
		write_byte(cvar_nvgcolor[2]) // b
	}
...
public off(id)
{
	...
	// Human / Spectator in normal round
	else if (g_class[id] < ZOMBIE || !g_isalive[id])
	{
		write_byte(cvar_humnvgcolor[0]) // r
		write_byte(cvar_humnvgcolor[1]) // g
		write_byte(cvar_humnvgcolor[2]) // b
...
SetExp(index, iExp)
{
    ...
    static iRango, reqRango; iRango = g_iRango[index];
    ...
    reqRango = g_iRango[index] >= charsmax(rango) ? charsmax(rango) : g_iRango[index];
    while(g_iLevel[index] >= rango[reqRango][range_level]) ++g_iRango[index];
...
RefreshHH()
{
    ...
    for( i = 0 ; i < sizeof _HappyHour ; ++i )
    {
        if(equal(_HappyHour[i][HH_HOUR], current_hour))
        {
            ...
            zp_colored_print(0, "^x04[ZP] ^x04HORA FELIZ ACTIVA!!! ^x03Daño requerido: ^x01%d ^x03Multiplicador: ^x01%d", g_iDefaultDamage, g_iHappyMulti);
            break;
        }
    }
}
...
public clcmd_say(id)
{
	...
	else formatex(class, charsmax(class), "%s", rango[g_iRango[id]][range_name])
