...
new const zclass_info[] = { "Press G to Traps" }
...
new const zclass_health = 9500
new const zclass_speed = 250
...
public plugin_init() 
{
	...
	cvar_skillcooldawntime = register_cvar("zp_undertaker_cooldawn","8.0") //Р¶Р‰Р‚РёС“Р…РµвЂ В·РµРЊТ‘Р¶вЂ”В¶Р№вЂ”Т‘
	cvar_skillhealth = register_cvar("zp_undertaker_skillhealth","800") //Р№вЂњРѓРµВ¤вЂћРµТђС–Р·С™вЂћР·вЂќСџРµвЂР…РµР‚С
	cvar_coffintime = register_cvar("zp_undertaker_coffintime","10.0") //Р№вЂњРѓРµВ¤вЂћРµТђС–Р·С™вЂћРµВ­ВРµСљРЃР¶вЂ”В¶Р№вЂ”Т‘
	cvar_exrange = register_cvar("zp_undertaker_coffinexrange","150") //Р№вЂњРѓРµВ¤вЂћРµТђС–Р·в‚¬вЂ Р·вЂљС‘РµР…В±РµвЂњРЊРёРЉС“РµвЂєТ‘
	cvar_exknock = register_cvar("zp_undertaker_coffinexknock","1000") //Р№вЂњРѓРµВ¤вЂћРµТђС–Р·в‚¬вЂ Р·вЂљС‘РµвЂЎВ»Р№Р‚Р‚
	cvar_speedtime = register_cvar("zp_undertaker_speedtime","5.0") //Р№вЂњРѓРµВ¤вЂћРµТђС–Р№Р‚СџРµС”В¦РµР…В±РµвЂњРЊР¶вЂ”В¶Р№вЂ”Т‘
	cvar_maxspeed = register_cvar("zp_undertaker_maxspeed","0.5") //РµРЏвЂ”РµР…В±РµвЂњРЊР¶вЂ”В¶Р·С™вЂћРёРЋРЉРёВµВ°Р№Р‚СџРµС”В¦
...
public makecoffin(id)
{
	...
	if(is_user_alive(id) && zp_get_user_zombie(id) && zp_get_user_zombie_class(id) == g_zclass_undertaker)
	{
	skilluse(id)
	return PLUGIN_HANDLED
	}
...
public skilluse(id)
{
	if(zp_get_user_zombie(id) && zp_get_user_zombie_class(id) == g_zclass_undertaker && zp_get_class(id) >= ZOMBIE && zp_get_class(id) < NEMESIS && !skillcooldawn[id] && !skilluseing[id] && get_user_weapon(id) == CSW_KNIFE)
	{
		...
		else client_print(id,print_center,"Способность Готова!")
	}
}
...
public skillcooldawnover(taskid)
{
	...
	client_print(id,print_center,"Способность Готова!")
...
public settheent(taskid)
{
	...
	if(!zp_get_user_zombie(id) || zp_get_user_zombie_class(id) != g_zclass_undertaker || zp_get_class(id) >= NEMESIS)
	return
...
{
     // РµвЂ вЂ¦РµВ®в„–Р№в„ўС’РµВ®С™РµРЏР„РµР‡в„–РµС“ВµРµВ°С‘РґР…СљР·вЂќРЃ
     if (!is_user_connected(id) || !zp_get_user_zombie(id) || zp_get_user_zombie_class(id) != g_zclass_undertaker)
           return FMRES_IGNORED;
     
     // РµС“ВµРµВ°С‘РёСћВ«Р¶вЂќВ»РµвЂЎВ»Р·С™вЂћРµРЏВ«РµР€В°
     ...
     // РµС“ВµРµВ°С‘Р·вЂќРЃР·в‚¬Р„РµВ­С’Р¶вЂќВ»РµвЂЎВ»Р·С™вЂћРµР€В°Р№СџС–
     if (equal(sample[8], "kni", 3))
     {
           if (equal(sample[14], "sla", 3)) // Р·в‚¬Р„РµВ­С’Р·В©С”Р¶РЉТђР·С™вЂћР№СџС–Р¶вЂўв‚¬
           { 
                 engfunc(EngFunc_EmitSound, id, channel, "zombie_plague/knife_slash1.wav", volume, attn, flags, pitch)
                 return FMRES_SUPERCEDE;
           }
           if (equal(sample[14], "hit", 3)) // Р·в‚¬Р„РµВ­С’РµвЂР…РґС‘В­Р·вЂ°В©РґР…вЂњР·С™вЂћР№СџС–Р¶вЂўв‚¬
           {
                 if (sample[17] == 'w') // Р·в‚¬Р„РµВ­С’РµвЂР…РґС‘В­Р·С™вЂћР¶ВР‡РµСћв„ўРµР€Рѓ
                 {
                       engfunc(EngFunc_EmitSound, id, channel, "zombie_plague/knife_hitwall1.wav", volume, attn, flags, pitch) 
                       return FMRES_SUPERCEDE;
                 }
                 else // Р·в‚¬Р„РµВ­С’РµвЂР…РґС‘В­Р·С™вЂћР¶ВР‡РґС”С”Р·вЂ°В©
                 { 
                       engfunc(EngFunc_EmitSound, id, channel, "zombie_plague/knife_hit1.wav", volume, attn, flags, pitch) 
                       return FMRES_SUPERCEDE;
                 }
           }
           if (equal(sample[14], "sta", 3)) // Р·в‚¬Р„РµВ­С’Р№вЂЎРЊР·В РЊРµвЂР…РґС‘В­РґС”С”Р·вЂ°В©
           {
     ...
     // РµС“ВµРµВ°С‘Р¶В­В»РґС”РЋР·С™вЂћРµР€В°Р№СџС–
     ...
     // РµС“ВµРµВ°С‘Р¶вЂвЂќРґСВ¤Р·С™вЂћРµР€В°Р№СџС–
