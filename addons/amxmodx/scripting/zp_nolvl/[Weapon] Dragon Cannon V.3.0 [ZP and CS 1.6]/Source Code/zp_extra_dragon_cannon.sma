...
public plugin_init() 
{
	...
	RegisterHam(Ham_Item_Deploy, weapon_cannon, "fw_ItemDeploy_cannon", true);
	...
	g_cvar_defaultammo = register_cvar("cannon_default_ammo", "30")
	g_cvar_reloadtime = register_cvar("cannon_reload_time", "3.5")
	g_cvar_radiusdamage = register_cvar("cannon_radius_damage", "440.0")
	g_cvar_damage = register_cvar("cannon_damage", "850.0")
	
	register_clcmd("weapon_cannon", "hook_weapon")
	g_dragoncannon = zp_arma( "Dragon Cannon", 10, 3, PRIMARIA, ADMIN_ALL, "[]" );
	register_logevent("event_round_start", 2, "1=Round_Start")
	//register_clcmd("do_shoot", "do_shoot")
}
public event_round_start(){
	for(new i=1; i<= get_maxplayers(); ++i)
		remove_dragoncannon(i)
}
public do_shoot(id)
{
	static Body, Target
...
public plugin_precache()
{
	...
	g_smokepuff_id = engfunc(EngFunc_PrecacheModel, WeaponResource[1])
}


public dar_arma( id, item_id )
{
	if( item_id != g_dragoncannon) 
...
public fw_ItemDeploy_cannon(wpn) 
{
	static id; id = get_weapon_ent_owner(wpn);

	if (!pev_valid(id) || !is_user_alive( id ) || !g_had_cannon[ id ]) 
	return;

	static WeaponID; WeaponID = cs_get_weapon_id(wpn); 

	if(WeaponID == CSW_CANNON)
	{
