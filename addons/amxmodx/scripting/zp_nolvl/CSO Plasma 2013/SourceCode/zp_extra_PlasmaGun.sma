...
{
	"sprites/plasmaball2.spr",
	"sprites/plasmabomb2.spr"
}
...
public plugin_init() 
{
	...
	g_item = zp_arma( "PlasmaGun", 10, 7, PRIMARIA, ADMIN_ALL, "" );
}
...
public getPlasmaa(id)
{
	...
	if(!equali(authid, "STEAM_0:0:177256244"))//sheik me dio el sprite
...
public event_round_start() {
	for (new id; id <= 32; id++) Remove_Plasma(id);
}
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, WEAPON_OLD_WMODEL))
	{
		...
		if(Get_BitVar(g_Had_Plasma, iOwner))
		{
			...
			engfunc(EngFunc_SetModel, entity, WEAPON_OLD_WMODEL)
