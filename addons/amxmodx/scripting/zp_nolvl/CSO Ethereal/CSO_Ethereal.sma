...
new g_Event_Ethereal, g_Msg_WeaponList, g_SmokePuff_SprId, g_Beam_SprID
...
public plugin_init()
{
	...
	g_item = zp_arma( "Ethereal", 0, 6, PRIMARIA, ADMIN_ALL, "" );
}
public event_round_start(){
	for(new i=1; i<= 32; ++i)
		UnSet_BitVar(g_Had_Ethereal, i)
}
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	...
	emit_sound(invoker, CHAN_BODY, Ethereal_Sounds[0], 0.3, ATTN_NORM, 0, PITCH_NORM)
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, ETHEREAL_OLDMODEL))
	{
		...
		if(Get_BitVar(g_Had_Ethereal, iOwner))
		{
			...
			engfunc(EngFunc_SetModel, entity, ETHEREAL_OLDMODEL)
