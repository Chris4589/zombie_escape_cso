...
public Event_CurWeapon(id)
{
	if(!is_user_alive(id))
		return
	if(!g_Had_DualKatana[id] || zp_get_class(id) >= ZOMBIE)
		return
...
public fw_CmdStart(id, uc_handle, seed)
{
	if (!is_user_alive(id)) 
		return
	if(get_user_weapon(id) != CSW_DUALKATANA || !g_Had_DualKatana[id] || zp_get_class(id) >= ZOMBIE)
		return
...
public Do_Slashing(id)
{
	...
	{
		emit_sound(id, CHAN_WEAPON, DualKatana_Sound[1], 1.0, ATTN_NORM, 0, PITCH_NORM)
		do_attack(id, Target, Ent, float(SLASH_DAMAGE))
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, DualKatana_Sound[2], 1.0, ATTN_NORM, 0, PITCH_NORM)
		else if(g_Hit_Ing[id] == HIT_NOTHING) emit_sound(id, CHAN_WEAPON, DualKatana_Sound[3], 1.0, ATTN_NORM, 0, PITCH_NORM)	
	}
...
public Do_StabNow(id)
{
	...
	{
		emit_sound(id, CHAN_WEAPON, DualKatana_Sound[4], 1.0, ATTN_NORM, 0, PITCH_NORM)
	} else {
		if(g_Hit_Ing[id] == HIT_WALL) emit_sound(id, CHAN_WEAPON, DualKatana_Sound[2], 1.0, ATTN_NORM, 0, PITCH_NORM)
		else if(g_Hit_Ing[id] == HIT_NOTHING) emit_sound(id, CHAN_WEAPON, DualKatana_Sound[5], 1.0, ATTN_NORM, 0, PITCH_NORM)
	}
...
public fw_PlayerTraceAttack(Victim, Attacker, Float:Damage, Float:Direction[3], TraceResult, DamageBits) 
{
	if(!is_user_alive(Attacker))	
		return HAM_IGNORED
	if(!g_Had_DualKatana[Attacker] || !g_Checking_Mode[Attacker] || zp_get_class(Attacker) >= ZOMBIE)
		return HAM_IGNORED
