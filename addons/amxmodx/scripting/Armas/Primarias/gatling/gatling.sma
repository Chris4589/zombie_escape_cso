...
public plugin_init() 
{
	...
	g_item = zp_arma( "Gatling", 0, 4, PRIMARIA, ADMIN_BAN, "[SILVER]" );
...
public event_round_start() {
	for (new id; id <= 32; id++) g_had_gatling[id] = false;
}

public dar_arma(id, item) {
	if( g_item != item) return;
	get_gatling(id)
}
...
public fw_Item_AddToPlayer_Post(ent, id)
{
	...
	write_string(g_had_gatling[id] == 1 ? "weapon_m3" : "weapon_m3")
	write_byte(5)
	write_byte(200)
	write_byte(-1)
	write_byte(-1)
