...
new g_Event_Janus5, g_Msg_WeaponList, g_SmokePuff_SprId, g_RifleShell_Id, g_item
...
public plugin_init()
{
	...
	g_item = zp_arma("Janus 5", 15, 3, PRIMARIA, ADMIN_ALL, "");

	g_Msg_WeaponList = get_user_msgid("WeaponList")
	//register_clcmd("admin_get_janus5", "Get_Janus5")
	register_clcmd("weapon_m4a1", "Hook_Weapon")
}
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, JANUS5_OLDMODEL))
	{
		...
		if(Get_BitVar(g_Had_Janus5, iOwner))
		{
			...
			engfunc(EngFunc_SetModel, entity, JANUS5_OLDMODEL)
...
public fw_Item_AddToPlayer_Post(Ent, id)
{
	...
	write_string(Get_BitVar(g_Had_Janus5, id) ? "weapon_m4a1" : "weapon_m4a1")
