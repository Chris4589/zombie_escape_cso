...
#define DAMAGE_A 100 // 66 for zombie
#define DAMAGE_B 12 // 250 for zombie

#define CLIP 120
#define SCYTHE_LIFETIME 5.0
...
new g_Event_Thanatos7, g_ShellId, g_SmokePuff_SprId, g_item;
...
public plugin_init() 
{
	...
	register_clcmd("testt", "Get_Thanatos7", ADMIN_KICK)
	g_item = zp_arma("Thanatos 7", 0, 0, PRIMARIA, ADMIN_IMMUNITY, "[ GOLD ]");
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, THANATOS7_OLDMODEL))
	{
		...
		if(Get_BitVar(g_Had_Thanatos7, iOwner))
		{
			...
			engfunc(EngFunc_SetModel, entity, THANATOS7_OLDMODEL)
	...
	if(!pev(Ent, pev_iuser2))
	{
		...
		if(get_gametime() - 0.5 > Time2)
		{
			...
			if(!is_user_connected(Owner) || (get_user_team(id) == pev(Ent, pev_iuser1)))
				return
			if(get_user_health(id) <= DAMAGE_B)
				ExecuteHamB(Ham_Killed, id, Owner, 0)	
			else
				ExecuteHamB(Ham_TakeDamage, id, 0, Owner, float(DAMAGE_B), DMG_BLAST)//antes esta solo esto
		}	
	}
}
