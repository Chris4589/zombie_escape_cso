...
new g_Event_Janus7, g_Msg_WeaponList, g_SmokePuff_SprId, g_RifleShell_Id, g_SprId, g_MaxPlayers, g_item
...
public plugin_init()
{
	...
	g_item = zp_arma("Janus 7", 0, 0, PRIMARIA, ADMIN_IMMUNITY, "[ GOLD ]");	
}
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, Janus7_OLDMODEL))
	{
		...
		if(Get_BitVar(g_Had_Janus7, iOwner))
		{
			...
			engfunc(EngFunc_SetModel, entity, Janus7_OLDMODEL)
...
public fw_Item_AddToPlayer_Post(Ent, id)
{
	...
	write_string(Get_BitVar(g_Had_Janus7, id) ? "weapon_m249" : "weapon_m249")
