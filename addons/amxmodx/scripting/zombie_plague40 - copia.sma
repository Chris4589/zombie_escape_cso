...
new const MYSQL_HOST[] =  "127.0.0.1";//127.0.0.1
new const MYSQL_USER[] =  "root"; //root
new const MYSQL_PASS[] =  ""; //DeivizEdition2020
new const MYSQL_DATEBASE[] =  "zombie_plague"; //zombie_plague

new const g_szCuentas[ ] = "zp_Cuentas";
...
enum { REGISTRAR_CUENTA, LOGUEAR_CUENTA, GUARDAR_DATOS, CARGAR_DATOS, SQL_RANK, SQL_TOP };
...
new const szZombies[][] = { "zombie_source" }
...
public plugin_precache()
{
	...
	for( i = 0; i < sizeof(szZombies); ++i )
	{
		prechache_player_model(szZombies[i]);
	}
	for( i = 0; i < sizeof(szZombies_knife); ++i )
		precache_model(szZombies_knife[i]);

	prechache_player_model(szNemesis);
...
public plugin_init()
{
	...
	register_clcmd("login_bots", "func_login_bots")
...
public logevent_round_end()
{
	...
	if (iNum > 2)
	{
		...
		for(Count = 0; Count < 5; Count++)
		{
		    ...
		    iLen += formatex(szTopPlayers[iLen], charsmax(szTopPlayers) - iLen, "%d. %s (%d Exp),^n", Count+1, g_playername[id], g_tempApps[id])
		   
		    if(g_temExp[id] > 0)
		    {
		        g_ammopacks[id] += 15;
		        client_print(id, print_chat, "[ZP] Ganaste 15 Ammo Packs por ser el Numero %d de los mejores de la ronda.", Count+1);
		    }
		    Count++;
		}
...
{
	oldmenu_create( "login_menu", "BIENVENIDOS AL \yZOMBIE PLAGUE v1.0^nVersion: \r1.0 \w- \yCreador: \rdivstarproject.com" );
...
public login_menu( id, item, value, page )
{
	...
	switch( item ) 
	{
		...
		case 2: client_cmd( id, "messagemode LOGUEAR_PASSWORD" ); 
	}
...
public checkTop( id )
{
	...
	iData[ 0 ] = id;
	iData[ 1 ] = SQL_TOP;
	formatex( szTabla, charsmax( szTabla ), "SELECT Personaje, rango, level, reset FROM %s ORDER BY rango DESC, level DESC, reset DESC LIMIT 10", g_szCuentas );
...
public checkRank( id )
{
	...
	iData[ 0 ] = id;
	iData[ 1 ] = SQL_RANK;
	formatex( szTabla, charsmax( szTabla ), "SELECT (COUNT(*) + 1) FROM `%s` WHERE `reset` > '%d' OR (`reset` = '%d' AND `level` > '%d')", g_szCuentas, g_iReset[ id ], g_iReset[ id ], g_iLevel[ id ] );
	mysql_query( g_hConnection, "DataHandler", szTabla, iData, 2 );
...
public reg_password( id ) 
{
	...
	get_user_name( id, g_playername[ id ], charsmax( g_playername[ ] ) );
	#if AMXX_VERSION_NUM < 183
	formatex( szQuery, charsmax( szQuery ), "INSERT INTO %s (Personaje, Password) VALUES (^"%s^", ^"%s^")", g_szCuentas, g_playername[ id ], g_password[ id ] );
	#else
	formatex( szQuery, charsmax( szQuery ), "INSERT INTO %s (Personaje, Password) VALUES (^"%s^", ^"%s^")", g_szCuentas, g_playername[ id ], g_szHashPass[ id ] );
	#endif
...
public log_password( id ) 
{
	...
	#if AMXX_VERSION_NUM < 183
	formatex( szQuery, charsmax( szQuery ), "SELECT * FROM %s WHERE Personaje=^"%s^" AND Password=^"%s^"", g_szCuentas, g_playername[ id ], g_password[ id ] );
	#else
	formatex( szQuery, charsmax( szQuery ), "SELECT * FROM %s WHERE Personaje=^"%s^" AND Password=^"%s^"", g_szCuentas, g_playername[ id ], g_szHashPass[ id ] );
	#endif
	mysql_query( g_hConnection, "DataHandler", szQuery, iData, 2 );
...
public guardar_datos( id ) 
{
	...
	formatex( szQuery, charsmax( szQuery ), "UPDATE %s SET level=%d, reset=%d, exp=%d, rango=%d, ammopacks=%d, hud=%d, nvision=%d, hat=%d WHERE idCuenta ='%d' AND Personaje = ^"%s^"", 
	g_szCuentas, g_iLevel[ id ], g_iReset[ id ], g_iExp[ id ], g_iRango[ id ], g_ammopacks[ id ], g_iHud[ id ], g_iNVsion[ id ], g_iHat[ id ], g_id[ id ], g_playername[ id ] );
...
public DataHandler( failstate, error[], error2, data[], size, Float:queuetime )
{
	...
	switch( data[ 1 ] ) 
	{
		case REGISTRAR_CUENTA: 
		{
			...
			{
				if( containi( error, "Pj" ) != -1 ) zp_colored_print( id,  "%s El nombre de personaje esta en uso.", g_szPrefix );
				else zp_colored_print( id,  "%s Error al crear la cuenta. Intente nuevamente.", g_szPrefix );
			...
			else 
			{
				...
				#if AMXX_VERSION_NUM < 183
				formatex( szQuery, charsmax( szQuery ), "SELECT idCuenta FROM %s WHERE Personaje=^"%s^"", g_szCuentas, g_playername[ id ] );
				#else
				formatex( szQuery, charsmax( szQuery ), "SELECT idCuenta FROM %s WHERE Personaje=^"%s^"", g_szCuentas, g_playername[ id ] );
				#endif
		...
		case LOGUEAR_CUENTA: 
		{
			if( mysql_num_results( ) ) 
			{
				...
				mysql_read_result( 1, g_playername[ id ], charsmax( g_playername[ ] ) );
				g_iLevel[ id ] = mysql_read_result( 3 );
				g_iReset[ id ] = mysql_read_result( 4 );
				g_iExp[ id ] = mysql_read_result( 5 );
				g_iRango[ id ] = mysql_read_result( 6 );
				g_ammopacks[ id ] = mysql_read_result( 7 );
				g_iHud[ id ] = mysql_read_result( 8 );
				g_iNVsion[ id ] = mysql_read_result( 9 );
				g_iHat[ id ] = mysql_read_result( 10 );
			...
			{
				zp_colored_print( id,  "[ %s ] Usuario o ContraseÃ± incorrecta.", g_szPrefix );
				client_cmd( id, "spk buttons/button10.wav" );
		...
		case CARGAR_DATOS:
		{
			...
			{
				zp_colored_print( id,  "%s Error al cargar los datos, intente nuevamente.", g_szPrefix );
				show_login_menu( id );
			}
		...
		{
			if( mysql_num_results() )
				zp_colored_print( id,  "^4%s^1 Tu Rank es ^4%i", g_szPrefix, mysql_read_result( 0 ) );
		}
		case SQL_TOP:
		{
			new len = 0, szBuffer[ 1024 ], iPosition, szName[ 33 ], iRng, reset, lvl;
			if( mysql_num_results() )
			{
				...
				len += format(szBuffer[len], charsmax(szBuffer) - len, "<tr  align=center bgcolor=#52697B><th width=5%%> # <th width=22%% align=left> Personaje <th width=10%%> Reset <th width=13%%> Insignea <th width=13%%> Level");
				...
				while( mysql_more_results( ) )
				{
					...
					lvl = mysql_read_result( 2 );
					reset = mysql_read_result( 3 );

					len += format( szBuffer[ len ], charsmax( szBuffer ) - len, "<tr align=center%s><td> %d <td align=left> %s <td> %d <td> <img src= ^"%s^"> <td> %d", ((iPosition%2)==0) ? "" : " bgcolor=#2f3030", iPosition, szName, reset, rango[ iRng >= charsmax(rango) ? charsmax(rango) : iRng ][ url_range ], lvl );
					
					mysql_next_row( );
				}
				show_motd( id, szBuffer, "Top 10" );
			}
			else
			{
				zp_colored_print( id,  "^4%s^1 EL TOP10 ESTA VACIO", g_szPrefix );
			}
		}
	}
}
...
public func_login_success( id )
{
	if( is_user_connected(id) )
	{
		...
		g_tempDamage[id] = g_tempApps[id] = g_temExp[id] = 0;
...
public MySQLx_Init( )
{
	...
	len += formatex(szTabla[len], charsmax(szTabla) - len, "Personaje varchar(34) not null UNIQUE KEY,");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "Password varchar(34) NOT NULL,");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "level int(10) NOT NULL DEFAULT '0',");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "reset int(10) NOT NULL DEFAULT '0',");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "exp int(10) NOT NULL DEFAULT '0',");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "rango int(10) NOT NULL DEFAULT '0',");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "ammopacks int(10) NOT NULL DEFAULT '0',");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "hud int(10) NOT NULL DEFAULT '0',");
	len += formatex(szTabla[len], charsmax(szTabla) - len, "nvision int(10) NOT NULL DEFAULT '0',");
	len += format(szTabla[len], charsmax(szTabla) - len, "hat int(10) NOT NULL )");
...
public handler_NVsion(id, menu, item){
	...
	g_iNVsion[id] = item;
	zp_colored_print(id, "^x04[ZP]^x01 Haz elegido el color ^x04%s", g_ColorNVsion[item][nvisionName]);
	return PLUGIN_HANDLED;
}
public menuHud(id)
{
...
public handler_color(id, menu, item){
	...
	g_iHud[id] = item;
	zp_colored_print(id, "^x04[ZP]^x01 Haz elegido el color ^x04%s", g_ColorHud[item][hudName]);
	return PLUGIN_HANDLED;
}
public listar_hh(id)
{
	...
	{
		formatex(info, charsmax(info), "\wHora: \r%s\w - Damage: \r%d\w - Multiplicador: \r%d\w", _HappyHour[i][HH_HOUR], _HappyHour[i][HH_DAMAGE], _HappyHour[i][HH_MULTI]);
...
public cmdMenu_config(id)
{
	...
	menu_additem(gMenu, "\wCambiar Color Hud", "3")
	menu_additem(gMenu, "\wCambiar Color NGVision", "4")
	menu_additem(gMenu, "\wParty \rAPS", "5")
	menu_additem(gMenu, "\wListar \rNiveles", "6")
	menu_additem(gMenu, "\wHorarios \rHappy Hour")
...
show_menu_game(id)
{
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\yDark Zone Community^n^n")
	
	// 1. Buy weapons
	if (get_pcvar_num(cvar_buycustom) && g_class[id] < SURVIVOR)
		len += formatex(menu[len], charsmax(menu) - len, "\r1. \wComprar Armas^n");
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r1. \dComprar Armas^n");
	
	// 2. Extra items
	if (get_pcvar_num(cvar_extraitems) && g_isalive[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r2. \wComprar Items Extra^n");
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d2. Comprar Items Extra^n")
	
	// 3. Zombie class
	if (get_pcvar_num(cvar_zclasses))
		len += formatex(menu[len], charsmax(menu) - len, "\r3. \wElegir Clases de Zombie^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\d3. Elegir Clases de Zombie^n")
	
	len += formatex(menu[len], charsmax(menu) - len, "\r4. \wHerramientas^n");
	
	// 5. Help
	//len += formatex(menu[len], charsmax(menu) - len, "\r5. \wZona \rPREMIUM^n")
	//6
	//len += formatex(menu[len], charsmax(menu) - len, "\r6. \yLogros^n")
	//7
	len += formatex(menu[len], charsmax(menu) - len, "\r6. \wTop Players^n")

	//8
	len += formatex(menu[len], charsmax(menu) - len, "\r7. \wRenta de admin^n^n")
	// 9. Admin menu
	if (is_user_admin(id))
...
public menu_game(id, key)
{
	switch (key)
	{
		case 0: // Buy Weapons
		{
			...
			if (get_pcvar_num(cvar_buycustom))
			{
				if(g_bAutoSeleccion[id]){
					g_bAutoSeleccion[id] = false;
					zp_colored_print(id, "^x04[ZP]^x01 Podras usar el menu de armas en la proxima ronda")
				}
		...
		{
			//show_menu_info(id)
			zp_colored_print(id, "^x04[ZP]^x01 Proximamente!")
		}
		case 5: // logros
		{
			checkTop(id);
...
zombieme(id, infector, nemesis, silentmode, rewards)
{
	...
	{
		//modelzm
		formatex(g_playermodel[id], charsmax(g_playermodel[]), "%s", szZombies[random_num(0, charsmax(szZombies))]);
		cs_set_user_model(id, g_playermodel[id])
	}
...
public ShowHUD(taskid)
{
	...
	else
	{
		...
		ShowSyncHudMsg(ID_SHOWHUD, g_MsgSync2, "Vida: %d^nClase: %s^nExperiencia: %d/%d [ %i%% ]^nNivel: %d/%d || Reset: %d^nAmmo packs: %d^nDamage: %d/%d^n^n%s", 
			get_user_health(id), class, g_iExp[id], RequiredExp[lvl], porcentaje(float(g_iLevel[id]), float(MAX_LEVEL)), g_iLevel[id], MAX_LEVEL, g_iReset[id], g_ammopacks[id], g_iDamage[id], g_iDefaultDamage, PartyMsg);
	}
}
	...
	if(g_ammopacks[id] >= cost)
	{
		switch(command)
		{
			case 0:{
				...
				zp_colored_print(0, "^x04[ZP]^x01 El Admin^x04 %s^x01 convirtio a^x04 %s^x01 en ^x04Zombie", g_playername[id], g_playername[player] );
			}
			case 1:{
				humanme(player, 0, 0)
			}
			case 2:{
				...
				zp_colored_print(0, "^x04[ZP]^x01 El Admin^x04 %s^x01 convirtio a^x04 %s^x01 en ^x04Humano", g_playername[id], g_playername[player] );
			}
			case 3:{
				// New round?
				if (g_newround)
				...
				zp_colored_print(0, "^x04[ZP]^x01 El Admin^x04 %s^x01 convirtio a^x04 %s^x01 en ^x04Nemesis", g_playername[id], g_playername[player] );
			}
			case 4:{
				// Respawn as zombie?
				if (get_pcvar_num(cvar_deathmatch) == 2 || (get_pcvar_num(cvar_deathmatch) == 3 && random_num(0, 1)) || (get_pcvar_num(cvar_deathmatch) == 4 && fnGetZombies() < fnGetAlive()/2))
				...
				zp_colored_print(0, "^x04[ZP]^x01 El Admin^x04 %s^x01 ha revivido a^x04 %s^x01", g_playername[id], g_playername[player] );
			}
			case 5:
			{
				if (g_newround) 
				...
				zp_colored_print(0, "^x04[ZP]^x01 El Admin^x04 %s^x01 convirtio a^x04 %s^x01 en ^x04Sniper", g_playername[id], g_playername[player] );
			}
			case 6:
			{
				if (g_newround) 
				...
				zp_colored_print(0, "^x04[ZP]^x01 El Admin^x04 %s^x01 convirtio a^x04 %s^x01 en ^x04Wesker", g_playername[id], g_playername[player] );
			}
	...
	if(g_ammopacks[id] >= cost)
	{
		switch(command)
		{
			case 0:{
				...
				zp_colored_print(0, "^x04[ZP] ^x01El Admin ^x04%s ^x01ejecutado el modo ^x04SWARM", g_playername[id]);
			}
			case 1:{
				// Call Multi Infection
				remove_task(TASK_MAKEZOMBIE)
				make_a_zombie(MODE_MULTI, 0)
				zp_colored_print(0, "^x04[ZP] ^x01El Admin ^x04%s ^x01ejecutado el modo ^x04INFECCION MULTIPLE", g_playername[id]);
			}
			case 2:{
				// Call Plague Mode
				remove_task(TASK_MAKEZOMBIE)
				make_a_zombie(MODE_PLAGUE, 0)
				zp_colored_print(0, "^x04[ZP] ^x01El Admin ^x04%s ^x01ejecutado el modo ^x04PLAGUE", g_playername[id]);
			}
		}
		g_ammopacks[id] -= cost;
	}
...
public native_register_zombie_class(const name[], const info[], level, hp, speed, Float:gravity, Float:knockback)
{
	// ZP disabled
	if (!g_pluginenabled)
		return -1;
...
SetExp(index, iExp)
{
	...
	if(g_iLevel[index] > iLevel) zp_colored_print(index, "^x04[ZP] ^x01Felicidades! Subiste al nivel ^x04%d", g_iLevel[index]);
	...
	if(g_iRango[index] > iRango) zp_colored_print(index, "^x04[ZP] ^x01Felicidades! Subiste al Rango ^x04%s", rango[g_iRango[index]][range_name]);
...
public FuncReset(index)
{
    ...
    zp_colored_print(index, "^x04[ZP] ^x01Felicidades! ^x04Reseteaste ^x01tu cuenta, ahora eres reset ^x04%d", g_iReset[index]);
    return PLUGIN_HANDLED;
}
RefreshHH()
{
    ...
    for( i = 0 ; i < sizeof _HappyHour ; ++i )
    {
        if(equal(_HappyHour[i][HH_HOUR], current_hour))
        {
            ...
            zp_colored_print(0, "^x04[ZP] ^x01HORA FELIZ ^x04ACTIVA!!! ^x01Daño requerido: ^x04%d ^x01Multiplicador: ^x04%d", g_iDefaultDamage, g_iHappyMulti)
            break;
        }
    }
}
...
public cmdMenu(id)
{
	...
	new gMenu = menu_create("\rWPN System", "handlerMenu")
...
public handler_skins( id, menu, item ) 
{
	...
	else 
	{
		...
		{
			zp_colored_print(id, "^x04[ZP] ^x01Compra un ^x04ADMIN^x01 para ese ^x04ITEM^x01");
			menu_armas( id, cat );
		}
	}
	return PLUGIN_HANDLED;
...
public show_party_menu(id) {
    ...
    new iMenu = menu_create("\r[ZP] \wMenu Party:","party_menu"), BlockParty[50]
...
public show_party_info_menu(id) {
    ...
    new iMenu = menu_create("\r[ZP] \wMenu Party:","party_info_menu")
...
public clcmd_say(id)
{
	...
	else formatex(class, charsmax(class), "%s", rango[rangee][range_name])
...
stock prechache_player_model( const modelname[] )
{
