...
new g_Event_Janus1, g_Msg_WeaponList, g_MaxPlayers, spr_trail, g_item
...
public plugin_init()
{
	...
	register_clcmd("weapon_deagle", "Hook_Weapon")

	register_event("HLTV", "Event_NewRound", "a", "1=0", "2=0")

	g_item = zp_arma("Janus I", 0, 0, SECUNDARIA, ADMIN_ALL, "");
}
...
public fw_SetModel(entity, model[])
{
	...
	if(equal(model, Janus1_OLDMODEL))
	{
		...
		if(Get_BitVar(g_Had_Janus1, iOwner))
		{
			...
			engfunc(EngFunc_SetModel, entity, Janus1_OLDMODEL)
...
public fw_Item_AddToPlayer_Post(Ent, id)
{
	...
	write_string(Get_BitVar(g_Had_Janus1, id) ? "weapon_deagle" : "weapon_deagle")
