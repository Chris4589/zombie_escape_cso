...
new g_exp_sprid, g_item_balrog9, g_exp_sprid2
...
public plugin_init() 
{
	...
	g_item_balrog9 = zp_arma("Balrog-IX", 0, 0, KNIFE, ADMIN_ALL, "");
...
public fw_TraceAttack_Post(ent, attacker, Float:Damage, Float:Dir[3], ptr, DamageType)
{
	...
	if(get_user_weapon(attacker) != CSW_KNIFE || !g_had_balrog9[attacker] || get_user_team(attacker) == get_user_team(ent))
		return HAM_IGNORED
		...
		if(sample[14] == 's' && sample[15] == 'l' && sample[16] == 'a')
		{
			...
			if(g_attack_mode[id] == MODE_CHARGE)
			{
				if(g_chargeattack[id] == 2)
					...
					emit_sound(id, channel, weapon_sound[random_num(B9_SLASH1, B9_SLASH1)], volume, attn, flags, pitch)
			} else emit_sound(id, channel, weapon_sound[random_num(B9_SLASH1, B9_SLASH1)], volume, attn, flags, pitch)
			...
			if (sample[17] == 'w') // wall
			{
				...
				if(g_attack_mode[id] == MODE_CHARGE)
				{
					if(g_chargeattack[id] == 2)
						...
						emit_sound(id, channel, weapon_sound[random_num(B9_HIT1, B9_HIT1)], volume, attn, flags, pitch)
				} else emit_sound(id, channel, weapon_sound[random_num(B9_HIT1, B9_HIT1)], volume, attn, flags, pitch)
				return FMRES_SUPERCEDE
			}
		}
